...
relField_list <- c(list(JSTOR = c(mf_left$numer_mf)), lapply(all_relevant_units, create.relField.object))
names(relField_list) <- c("JSTOR", all_relevant_units)
...
# write.table(alpha, file = "~/authorships/results/toplevel.csv", append = F, row.names = F,
#             sep = ",", col.names = T)

write.table(alpha, file = "toplevel.csv", append = F, row.names = F,
            sep = ",", col.names = T)
...
for(s in 1:sim.size){
  ...
  write.table(alpha, file = "toplevel.csv", append = T, row.names = F,
              sep = ",", col.names = F)
